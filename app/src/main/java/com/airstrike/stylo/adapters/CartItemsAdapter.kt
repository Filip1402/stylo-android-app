...
class CartItemsAdapter(private val cartItemsList: MutableList<CartItem>, private val itemRemovalCalback : CartItemListener) :
    ...
    inner class CartItemViewHolder(private val view: View) : RecyclerView.ViewHolder(view) {
        ...
        fun bind(item: CartItem) {
            ...
            cartMinusBtn.setOnClickListener {
                itemRemovalCalback.decreaseCartItemQuantity(item)
                notifyDataSetChanged()
            }
            cartPlusBtn.setOnClickListener {
                itemRemovalCalback.increaseCartItemQuantity(item)
                notifyDataSetChanged()

            }
            removeItemBtn.setOnClickListener {
                itemRemovalCalback.removeCartItem(item)
                notifyDataSetChanged()
            }
    ...
    override fun onCreateViewHolder(
        ...
        val cartItemView =
            LayoutInflater.from(parent.context).inflate(R.layout.shoping_cart_item_layout, parent, false)
        return CartItemViewHolder(cartItemView)
    }
